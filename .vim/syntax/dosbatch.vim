...
" Dosbat keywords
syn keyword dosbatchStatement	goto call exit sleep p4 sync resolve start grep gvim vim
...
syn match dosbatchString	"\<title\([^)>|]\|\^\@<=[)>|]\)*"lc=5 contains=dosbatchVariable,dosbatchArgument,dosbatchSpecialChar,@dosbatchNumber,@Spell
syn match dosbatchEchoOperator  "\<echo\s\+\(on\|off\)\s*$"lc=4

" Two slashes starts a file/folder path
syn region dosbatchPath	        start="\(//\|\\\\\)" end="\s\|$"
" If the path is started with a double quote, it doesn't end until a double quote
syn region dosbatchPath	        start="\"\(//\|\\\\\)" end="\"\|$"
" Drive letter, colon, slash or backslash starts a path
syn region dosbatchPath	        start="[a-zA-Z]:\(\\\|/\)" end="\s\|$"
" Again, quotes matter
syn region dosbatchPath	        start="\"[a-zA-Z]:\(\\\|/\)" end="\"\|$"
...
syn match dosbatchSwitch	"[/-]\(\S\+\|?\)" contains=dosbatchString,dosbatchOperator
...
syn match dosbatchLabel		"\<\(goto\|call\|start\)\s\+:\h\w*\>"lc=4
...
syn keyword dosbatchImplicit    date del dir diskcomp diskcopy doskey echo endlocal title
...
syn keyword dosbatchImplicit    sort subst time title tree type ver verify
