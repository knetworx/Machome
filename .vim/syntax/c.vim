...
"syn keyword	cLabel		case default sizeof sbrk sema_t sema_wait sema_init sema_post memcpy strcpy strdup strcmp memcmp
syn keyword     cLabel	        case default
syn keyword	cConditional	if else switch
syn keyword	cRepeat		while for do

" Specialized Keywords
"syn keyword	cMyKeywords	CMessageView
"syn keyword	cMyKeywords2	UBound NumBytes ByteVal Chex Resize SpacedBytes SetByte
"syn keyword	cLibKeywords	printf sprintf vsprintf
"syn keyword	cLibKeywords	open fopen close fclose gets fgets getc fgetc
"syn keyword	cLibKeywords	free malloc calloc alloc
"syn keyword	cLibKeywords	strlen strcat strcpy strtok strchr strrchr strpbrk

syn keyword	cTodo		contained TODO FIXME XXX NH
...
" Read the Custom Operator syntax to start with
if version < 600
  so <sfile>:p:h/custop.vim
else
  runtime! syntax/custop.vim
...
" Accept %: for # (C99)
syn region	cPreCondit	start="^\s*\(%:\|#\)\s*\(if\|ifdef\|ifndef\|elif\)\>" skip="\\$" end="$" end="//"me=s-1 contains=cComment,cCppString,cCharacter,cCppParen,cParenError,cNumbers,cCommentError,cSpaceError,cOperator
...
"syn match cLineSkip	"\\$"
syn cluster	cPreProcGroup	contains=cPreCondit,cIncluded,cInclude,cDefine,cErrInParen,cErrInBracket,cUserLabel,cOctalZero,cCppOut,cCppOut2,cCppSkip,cFormat,cNumber,cFloat,cOctal,cOctalError,cNumbersCom,cString,cCommentSkip,cCommentString,cComment2String,@cCommentGroup,cCommentStartError,cParen,cBracket,cMulti
syn region	cDefine		start="^\s*\(%:\|#\)\s*\(define\|undef\)\>" skip="\\$" end="$" end="//"me=s-1 contains=ALLBUT,@cPreProcGroup,@Spell,@cppRFGKeywords
...
syn cluster	cLabelGroup	contains=cUserLabel,cLabel
